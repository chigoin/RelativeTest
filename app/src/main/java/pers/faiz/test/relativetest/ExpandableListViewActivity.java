...
import pers.faiz.test.Adepter.ExListViewAdapter;
...
public class ExpandableListViewActivity extends AppCompatActivity {
    ...
    private ExListViewAdapter myAdapter = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        myAdapter = new ExListViewAdapter(gData, iData, mContext);
        exlist_lol.setAdapter(myAdapter);

        exlist_lol.setOnChildClickListener(new ExpandableListView.OnChildClickListener() {
            @Override
            public boolean onChildClick(ExpandableListView expandableListView, View view, int i, int i1, long l) {
                Toast.makeText(mContext,gData.get(i).getgName()+"组:"+iData.get(i).get(i1).getName(),Toast.LENGTH_SHORT).show();
                return true;
            }
        });
